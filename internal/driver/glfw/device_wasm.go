...
func init() {
	...
	dummyEntry.Call("addEventListener", "input", js.FuncOf(func(_ js.Value, args []js.Value) (result any) {
		...
		// operaciones de JS puras, sin tocar Fyne) y se encola como un
		// closure — nunca se llama a TypedRune/TypedKey desde el callback
		...
		// de Fyne, que es justo lo que terminó colgando la app. La cola
		// serializa: sólo pumpMainThreadWork (una goroutine que nosotros
		// creamos, el caso de uso soportado de fyne.DoAndWait, ver
		// thread.go) los despacha, uno por vez — ver también charInput y
		// keyPressed en window_wasm.go, que comparten esta misma cola
		// porque tienen exactamente el mismo problema con el teclado
		// físico (glfw-js dispara esos callbacks con `go` por cada tecla).
		//
		...
		// de DOM pura, no hace falta pasar por la cola.
		dummyEntry.Set("value", "")

		return nil
	}))

	go pumpMainThreadWork()
}

// activeCanvas es el canvas cuyo widget con foco recibe la entrada de
// teclado. Sólo hace falta guardar uno: en wasm, Fyne corre una única
...
// mainThreadWork es el único punto de entrada a las APIs de Fyne que mutan
// estado desde callbacks disparados por el navegador — teclado físico y
// virtual (#dummyEntry, arriba), scroll, resize, movimiento y click de
// mouse: glfw-js dispara TODOS estos callbacks con `go w.XCallback(...)`,
// una goroutine nueva por evento, así que todo lo que necesita tocar un
// widget enfocado se encola acá en vez de llamarse directo desde el
// callback de JS que lo generó.
var mainThreadWork = make(chan func(), 64)

func queueMainThreadWork(work func()) {
	select {
	case mainThreadWork <- work:
	default:
		// La cola llena significa eventos más rápido de lo que se puede
		...
		// que garantizar que ni un evento se pierda bajo una ráfaga
		// extrema.
		fyne.LogError("cola del hilo principal llena, se descartó un evento", nil)
	}
}

// pumpMainThreadWork es la única goroutine que traduce estos eventos en
// llamadas de Fyne — "una goroutine que nosotros creamos" es justo el
// caso de uso soportado por fyne.DoAndWait (thread.go). Al vaciar la cola
// de a un trabajo por vez, nunca hay dos despachos en vuelo al mismo
// tiempo, que es lo que un callback de JS por evento no puede garantizar
// bajo una ráfaga (tipeo rápido, scroll rápido, etc.).
func pumpMainThreadWork() {
	for work := range mainThreadWork {
		runOnMainThreadSafely(work)
	}
}

// runOnMainThreadSafely corre `work` en el hilo principal de Fyne. El
// recover tiene que vivir DENTRO del closure de DoAndWait, no alrededor
// del propio DoAndWait: `work` termina ejecutándose en la goroutine que
// Fyne usa para drenar su cola principal (runOnMainWithWait, loop.go), no
// en la que llamó a DoAndWait — un panic ahí se lleva puesto todo el
// programa sin importar qué recover haya en la goroutine que esperaba el
// resultado.
func runOnMainThreadSafely(work func()) {
	fyne.DoAndWait(func() {
		defer func() {
			if recovered := recover(); recovered != nil {
				fyne.LogError("panic despachando un evento encolado", nil)
			}
		}()
